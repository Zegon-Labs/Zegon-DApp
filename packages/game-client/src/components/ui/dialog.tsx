  ...
  return (
    <DialogPortal>
      ...
      <DialogPrimitive.Content
        className={cn(
          "bg-card data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 fixed top-[50%] left-[50%] z-50 grid w-full max-w-md translate-x-[-50%] translate-y-[-50%] gap-4 rounded-xl border border-border p-6 shadow-lg duration-200",
          className,
        )}
        {...props}
      >
...
function DialogTitle({
  ...
  return (
    <DialogPrimitive.Title
      className={cn("text-lg leading-none font-semibold text-primary", className)}
      {...props}
    />
  );
}
