...
function tauntForConfidence(confidence: number, rng: () => number): string {
  if (confidence >= 0.7) {
    return pickRandom(TAUNTS_HIGH, rng);
  }
  if (confidence >= 0.4) {
    return pickRandom(TAUNTS_MID, rng);
  }
  return pickRandom(TAUNTS_LOW, rng);
}
...
export class DummyZegonBrain implements IZegonBrain {
  ...
  async decide(ctx: RoundContext): Promise<ZegonDecision> {
    ...
    const taunt = tauntForConfidence(confidence, this.rng);
